...
export interface ButtonProps extends Omit<RACButtonProps, "formAction"> {
  ...
  formAction?: RACButtonProps["formAction"] | (() => Promise<void>);
}
...
let button = tv({
  ...
  variants: {
    variant: {
      ...
      ghost: "hover:bg-zinc-800 pressed:bg-zinc-900",
    },
    isDisabled: {
...
export function Button({ formAction, ...props }: ButtonProps) {
  return (
    <RACButton
      {...props}
      formAction={formAction as RACButtonProps["formAction"]}
